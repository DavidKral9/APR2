...
class Graph:
    ...
    def is_connected(self) -> bool:
        ...
        for node_id in self.node_ids():
            visit(node_id)
        print(visited)       
        return len(visited)+1 == len(stack)
        
    #chci se dostat z každýho bodu do každýho i přes sousedy
    def full_graph(n):
        if n < 1:
    ...
    def is_graph_full(self) -> bool:
        n = len(self)
        for node_id in self.node_ids():
            if self.node(node_id).out_degree != n-1:
                return False
        return True
    
    def triangle(self) -> bool:
        for a in self.node_ids():
            Na = set(self.node(a).neighbor_ids)
            for b in Na:
                if b <= a:
                    continue
                for c in self.node(b).neighbor_ids:
                    if c in Na:
                        return True
        return False
    
    def triagle_counter(self) -> int:
        count = 0
        for a in self.node_ids():
            Na = set(self.node(a).neighbor_ids)
            for b in Na:
    ...
    def check_sum_degree(graph) -> bool:
        if graph.type != GraphType.DIRECTED:
            raise ValueError("This test is only applicable to directed graphs")
        ...
        for node in graph:
            total_out_degree += node.out_degree
        for node in graph:
            total_in_degree += node.in_degree

        print(f"počet do:{total_in_degree}")
...
#                print(f"hrana {u} -> {v}")
#
#    def hrany_bez_duplicit(g):
#        for u in g.node_ids():
#           for v in g.node(u).neighbor_ids:
#            if u < v:  # nebo jiná podmínka pro eliminaci duplicit
#                print(f"Hrana {u} -- {v}")
#    def pocet_hran(g):
#        count = 0
#        for u in g.node_ids():
...
if __name__ == "__main__":
    ...
    #n = 5
    #f = Graph.full_graph(n)
    #print(f)
    #z = tree.is_graph_full() 
    #print(z)
